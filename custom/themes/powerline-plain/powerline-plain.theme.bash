...
function powerline_ansible_prompt {
    if [[ "$PATH" =~ "ansible" ]]; then
        ANSIVER=$(ansible --version | head -n1 | sed -e 's/ansible //' -e's/ last updated.*//')
        ANSIBLE_PROMPT="$(set_rgb_color - ${SCM_THEME_PROMPT_COLOR}) ansible ${ANSIVER} ${normal}"
    fi
}
...
function powerline_prompt_command() {
    ...
    powerline_ansible_prompt

    PS1="${SHELL_PROMPT}${VIRTUALENV_PROMPT}${SCM_PROMPT}${ANSIBLE_PROMPT}${CWD_PROMPT}${LAST_STATUS_PROMPT} "
}
